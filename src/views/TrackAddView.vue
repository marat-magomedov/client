  ...
  <div class="min-h-screen bg-light-bg dark:bg-gray-900 flex flex-col">
    ...
    <main class="flex-1 container mx-auto px-4 py-8 md:py-12">
      <div class="max-w-2xl mx-auto">
        ...
        <div v-if="tracksStore.error" class="mb-6 p-4 bg-red-400/10 rounded-xl border border-red-400/30 text-red-400">
          {{ tracksStore.error }}
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="bg-light-bg-secondary dark:bg-gray-800/40 backdrop-blur-lg p-6 md:p-8 rounded-2xl border border-light-border dark:border-gray-700/30 shadow-xl space-y-6 relative z-[10]">
          ...
          <div class="flex flex-col md:flex-row gap-4 justify-end">
            ...
            >
              ...
              <span v-else class="group-hover:translate-x-1 transition-transform">Создать трек</span>
            </button>
          </div>
        </form>
      </div>
...
async function handleSubmit() {
  if (!authStore.state.profile) return;
  ...
  const result = await tracksStore.addTrack(authStore.state.profile.id, {
    ...formData,
